...
<body>
    ...
    <form action="{{ route('noticias.update', $noticia->id) }}" method="POST">
        ...
        <select name="cat_noticia_id" required>
            ...
            @foreach($categories as $categoria)
                <option value="{{ $categoria->id }}" 
                    {{ old('cat_noticia_id', $noticia->cat_noticia_id) == $categoria->id ? 'selected' : '' }}>
                    {{ $categoria->nom }}
                </option>
            @endforeach
        </select>
        ...
        <input type="date" name="data_inicial" value="{{ old('data_inicial', $noticia->data_inicial ? \Carbon\Carbon::parse($noticia->data_inicial)->format('Y-m-d') : '') }}" required>

        <label>Data final:</label>
        <input type="date" name="data_final" value="{{ old('data_final', $noticia->data_final ? \Carbon\Carbon::parse($noticia->data_final)->format('Y-m-d') : '') }}" required>
    ...
    <form action="{{ route('noticias.destroy', $noticia->id) }}" method="POST">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-danger" onclick="return confirm('Segur que vols eliminar aquesta notícia?')">Eliminar</button>
    </form>
