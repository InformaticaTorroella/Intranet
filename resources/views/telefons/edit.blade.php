...
<x-header />
  <h2>Edita Telèfon</h2>
  <form method="POST" action="{{ route('telefons.update', $telefon->id) }}">
    @csrf @method('PUT')
    ...
    <input type="number" name="area" class="form-input" value="{{ $telefon->fk_id_area }}">

    <label class="form-label">Edifici</label>
    <input type="number" name="edifici" class="form-input" value="{{ $telefon->fk_id_equipament }}">
    ...
    <input type="date" name="data_edicio" class="form-input" value="{{ \Carbon\Carbon::parse($telefon->data_edicio)->format('Y-m-d') }}">
